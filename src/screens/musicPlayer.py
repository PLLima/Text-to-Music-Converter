...
class playerScreen(tk.Frame, Screen):
    ...
    def pressPlay(self):
        if self.playTrackButton:  # Check if the play track button is initialized
            ...
            self.playTrackButton.setText(newTextPT, calculateFontSize(TEXT_SCALES["TextButton"], self.getAppController().getScreenSize()))
            ...
            if self.downloadResetButton:
                ...
                self.downloadResetButton.setText(newTextDR, calculateFontSize(TEXT_SCALES["TextButton"], self.getAppController().getScreenSize()))

    def pressDownloadReset(self):
        currentDRText = self.downloadResetButton.getText()  # Get the current text of the Download/Reset track button
        if currentDRText == "Download": # Call Reset/Download based on button state
            self.download()
        else:
            self.reset()
